...
class HomeTab(QWidget):
    ...
    def _build_tracker_snapshot(self) -> TrackerSnapshot:
        banner_data = getattr(self, "banner_data", None)
        current_banner = None
        if isinstance(banner_data, dict):
            current_banner = CurrentBanner(
                name=str(banner_data.get("name", "")),
                image_url=banner_data.get("image_url"),
                image_bytes=banner_data.get("image_bytes"),
                starts_at=self._snapshot_datetime(banner_data.get("starts_at")),
                ends_at=self._snapshot_datetime(banner_data.get("ends_at")),
                rarity=banner_data.get("rarity"),
                element=banner_data.get("element"),
                source=banner_data.get("source"),
            )
        ...
        banners = tuple(
            BannerRecord(
                ...
                kind=record["kind"],
                starts_at=self._snapshot_datetime(record.get("starts_at")),
                ends_at=self._snapshot_datetime(record.get("ends_at")),
                image_url=record.get("image_url"),
                image_bytes=record.get("image_bytes"),
                rarity=record.get("rarity"),
                source=record.get("source"),
                is_speculated=record.get("is_speculated", record.get("speculated")),
            )
            for record in getattr(self, "_catalog_snapshot_source", ())
        )

        history = ConveneStorageManager().load()
        pulls = tuple(
            PullRecord(
                timestamp=self._snapshot_datetime(record.get("timestamp", record.get("time", record.get("date")))),
        ...
        pity_tracker = getattr(self, "pity_tracker", None)
        state = getattr(pity_tracker, "state", None)
        pity = PitySnapshot(
            resonator=getattr(state, "resonator", None),
            weapon=getattr(state, "weapon", None),
            standard_character=getattr(state, "standard_character", None),
            standard_weapon=getattr(state, "standard_weapon", None),
            five_star_history=tuple(getattr(state, "five_star_history", ())),
            recent_convene_details=tuple(getattr(state, "recent_convene_details", ())),
            total_registered=getattr(state, "total_registered", 0),
            four_star_total=getattr(state, "four_star_total", 0),
        )

        tracker_status = getattr(pity_tracker, "tracker_status", None)
        raw_pool_status = getattr(tracker_status, "pool_status", {})
        pool_status = {
            str(pool): PoolStatus(
                status=str(values.get("status", "unknown")),
                completed=bool(values.get("completed", False)),
                record_count=values.get("record_count", 0) if isinstance(values.get("record_count", 0), int) else 0,
                message=values.get("message") if isinstance(values.get("message"), str) else None,
            )
            for pool, values in raw_pool_status.items()
            if isinstance(values, dict)
        }
        status = TrackerStatusSnapshot(
            log_status=str(getattr(tracker_status, "log_status", "unknown")),
            api_status=str(getattr(tracker_status, "api_status", "unknown")),
            history_status=str(getattr(tracker_status, "history_status", "unknown")),
            sync_status=str(getattr(tracker_status, "sync_status", "idle")),
            last_sync_at=self._snapshot_datetime(getattr(tracker_status, "last_sync_at", None)),
            last_success_at=self._snapshot_datetime(getattr(tracker_status, "last_success_at", None)),
            history_age=getattr(tracker_status, "history_age", None),
            is_partial=getattr(tracker_status, "is_partial", None),
            is_stale=getattr(tracker_status, "is_stale", None),
            source=str(getattr(tracker_status, "source", "unknown")),
            message=str(getattr(tracker_status, "message", "")),
            new_records_count=getattr(tracker_status, "new_records_count", 0),
        ...
        notices = tuple(
            NoticeRecord(
                ...
                rewards=str(record.get("rewards", "")),
                image_url=record.get("image_url"),
            ...
            for record in getattr(pity_tracker, "notices", ())
            if isinstance(record, dict)
        )
        return TrackerSnapshot(current_banner, banners, pulls, pity, status, notices)
